<template>
  <DialogRoot v-model:open="open" :modal="true">
    ...
    <DialogPortal>
      ...
      />
      <DialogContent
        :class="`fixed left-1/2 top-1/2 z-[100] max-h-[85vh] ${width ?? 'w-[90vw]'} ${maxWidth ?? 'max-w-md'} -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-6 shadow-2xl focus:outline-none dark:bg-neutral-700`"
      >
        <DialogTitle
...
defineProps<{ maxWidth?: string; triggerClass?: string; width?: string }>();
